...
@router.post(
  "/create", response_model=TestResponse, dependencies=[Depends(require_roles("admin", "teacher"))]
)
...
@router.get(
  "/list", response_model=list[TestResponse], dependencies=[Depends(require_roles("admin"))]
)
...
async def list_by_course(
  ...
  return await handle_errors(
    lambda: service.get_by_lesson_id(user, lesson_id, delete_flg, skip, limit)
  )
...
async def list_active_tests(
  ...
  return await handle_errors(lambda: service.get_all_active(user, delete_flg, skip, limit))
...
@router.put(
  "/update", response_model=TestResponse, dependencies=[Depends(require_roles("admin", "teacher"))]
)
...
async def hard_delete_test(test_id: UUID, service: TestService = Depends(get_test_service)):
