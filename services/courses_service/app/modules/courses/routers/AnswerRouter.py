...
@router.get(
  "/list", response_model=list[AnswerResponse], dependencies=[Depends(require_roles("admin"))]
)
...
async def get_correct_answers_by_question(
  ...
  return await handle_errors(
    lambda: service.get_correct_answers_by_question(
      user, question_id, is_correct, delete_flg, skip, limit
    )
  )
...
async def hard_delete_answer(answer_id: UUID, service: AnswerService = Depends(get_answer_service)):
