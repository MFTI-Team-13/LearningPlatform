...
class LessonBase(BaseModel):
  ...
  order_index: int = Field(..., ge=1, le=1000, description="Порядковый номер")
  text_content: Optional[str] = Field(None, description="Текстовый контент")
  video_url: Optional[str] = Field(None, max_length=500, description="URL видео")
  ...
  def validate_content_consistency(self):
    ...
    if self.content_type == ContentType.VIDEO and not self.video_url:
...
class LessonUpdate(BaseModel):
  ...
  order_index: Optional[int] = Field(None, ge=1, le=1000)
