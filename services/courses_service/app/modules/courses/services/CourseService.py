...
class CourseService(BaseService):
    ...
    async def check_course_access(self, user:CurrentUser, course:Course):
        ...
        if "teacher" in roles:
            if course.author_id == user.id and course.delete_flg == False:
                return course
            raise ForbiddenError()

        if "student" in roles:
            assigned = await self.repo.get_assigned_to_user(user.id, course.id)
            if assigned:
                return course
            raise ForbiddenError()
    ...
    async def filter_courses_access(self, user:CurrentUser, courses: List[Course]):
        allowed = []

        for course in courses:
            try:
                checked = await self.check_course_access(user, course)
    ...
    async def create_course(self, author_id:UUID, in_data: CourseCreate) -> Course:
        ...
        return await super().create(dict_model)

    async def get_by_id_course(self,user:CurrentUser, id: UUID, delete_flg:bool | None):
        res = await super().get_by_id(id, delete_flg)

        course = await self.check_course_access(user, res)
        return course

    async def get_all_course(self, user:CurrentUser,delete_flg:bool | None, skip: int, limit: int):
        res = await super().get_all(delete_flg, skip, limit)
        courses = await self.filter_courses_access(user,res)
        return courses
    ...
    async def update_course(self, user:CurrentUser, id: UUID, in_data: CourseUpdate) -> Course:
        ...
        course = await self.check_course_access(user, course)
