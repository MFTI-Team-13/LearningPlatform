...
from app.modules.courses.schemas_import import QuestionCreate, QuestionResponse, QuestionUpdate
from app.modules.courses.services_import import QuestionService, get_question_service
...
@router.get(
  "/list", response_model=list[QuestionResponse], dependencies=[Depends(require_roles("admin"))]
)
...
async def get_by_test_and_order(
  ...
  return await handle_errors(
    lambda: service.get_by_test_and_order(user, test_id, order_index, delete_flg)
  )
...
async def get_by_type(
  ...
  return await handle_errors(
    lambda: service.get_by_question_type(question_type, delete_flg, skip, limit)
  )


@router.post(
  "/maxOrderIndex", response_model=int, dependencies=[Depends(require_roles("admin", "teacher"))]
)
...
@router.post(
  "/search", response_model=list[QuestionResponse], dependencies=[Depends(require_roles("admin"))]
)
async def search_in_test(
  ...
  return await handle_errors(
    lambda: service.search_in_test(user, test_id, query, delete_flg, skip, limit)
  )
...
async def hard_delete_test(test_id: UUID, service: QuestionService = Depends(get_question_service)):
