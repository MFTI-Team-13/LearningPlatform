...
@router.post(
  "/create", response_model=CourseReviewResponse, dependencies=[Depends(require_roles("student"))]
)
...
async def search_in_comments(
  ...
  return await handle_errors(
    lambda: service.search_in_comments(user, course_id, query, delete_flg, skip, limit)
  )
...
async def hard_delete_review(
  review_id: UUID, service: CourseReviewService = Depends(get_course_review_service)
):
...
@router.get(
  "/list", response_model=list[CourseReviewResponse], dependencies=[Depends(require_roles("admin"))]
)
